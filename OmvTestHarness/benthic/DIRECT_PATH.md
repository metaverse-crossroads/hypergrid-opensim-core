...
### 3.1 Apply the Patch
Use the provided `benthic_headless.patch` to inject the `crates/headless_client` code and update `Cargo.toml`.
...
```bash
cd metaverse_client
# Apply the patch (ensure it exists in ../)
...
### 5.3 Verify Output
Inspect `OmvTestHarness/benthic/mating_rituals.log`. You should see narrative entries like:
...
```bash
git add --force OmvTestHarness/benthic/benthic_headless.patch
git add --force OmvTestHarness/benthic/DIRECT_PATH.md
```
